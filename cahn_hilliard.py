...
# Initial average concentration from command line
c0 = float(sys.argv[1])
...
images = []
for t in range(0, maxIter + 1, skip):
    fname = os.path.join(image_File, f"c_{t}.png")
    if os.path.exists(fname):
        images.append(imageio.imread(fname))

video_path = os.path.join(os.getcwd(),  "cahn_hilliard_evolution.mp4")
imageio.mimsave(video_path, images, fps=10)  # Adjust fps for speed
